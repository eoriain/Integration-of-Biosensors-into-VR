...
public class LogBaselineStart : MonoBehaviour
{
    private CubeGrabbed controller;
    private bool header;

    // static log file names and formatters
    private static string LOGFILE_DIRECTORY = "BiometricLogFile";
    private static string LOGFILE_NAME_BASE = "_BaselineParticipantNo.csv";
    ...
    {
        header = true;
        controller = GameObject.Find("TargetObject").GetComponent<CubeGrabbed>();
    ...
    void Update()
    {
        ...
        if (header == true)
        {
            tw.WriteLine("Timestamp, Attention, Meditation, Delta, Theta, lowAlpha, highAlpha, lowBeta, highBeta, lowGamma, highGamma, Shimmergsr, ShimmerPPG, ShimmerHR, EmpaticaHR, EmpaticaTempurature, EmpaticaGSR, EmpaticaBVP, EmpaticaIBI, PressGo, PressExit, PickedUpObject, PlacedObject, AnvilPickedUpObject, AnvilPlacedObject");
            header = false;
        }        
